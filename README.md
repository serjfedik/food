...
- **Деплой Streamlit Cloud:** на скрине у вас выбран `Branch = main`, но код
  лежит на ветке `claude/sharp-wright-k5u79h`. Поменяйте в форме деплоя
  `Branch` на `claude/sharp-wright-k5u79h` (или скажите — выложу PR в main).
- **«О разработке» — финальный контент:** структура (4 вкладки + автор + гайд)
  готова и заполнена фактами проекта. Жду ваш MD с уточнениями: бизнес-цели,
  список конкурентов для сравнения, имя/название автора.
...
- **Внешние источники рецептов (без LLM):** пока только стартовый пак из 20
  рецептов локально. Под подключение Spoonacular/Edamam нужен ключ — скажете,
  каким сервисом готовы пользоваться, прикручу.
