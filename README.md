...
Streamlit-приложение для оцифровки и хранения рецептов с синхронизацией в Google Drive.
...
- **Адаптация** в реальном времени: правка артефактов распознавания и структуры
  (название / порции / время / ингредиенты / шаги / заметки).
- **Синхронизация** с Google Drive: сохранение как Google Doc, опционально
  `.md` и JSON-бэкап структуры.
- **Библиотека**: список рецептов в папке Drive, открытие любого обратно в редакторе.
...
```
food/
├── app.py                      # Streamlit UI и навигация
├── src/
│   ├── file_loader.py          # PDF / DOCX / image → текст
│   ├── ocr.py                  # Tesseract-обёртки
│   ├── recipe_parser.py        # эвристика текст → структура рецепта
│   └── google_drive.py         # синхронизация с Drive
├── requirements.txt
