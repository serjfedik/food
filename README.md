...
- **Google Drive + Sheets подключение** — пошаговая инструкция в
  `SETUP.md`, шаги 1-4. Жду ваших действий по чек-листу.
- **AI-OCR ключ (один из трёх)** — Tesseract в принципе не справляется
  с рукописным текстом, поэтому добавил поддержку Vision-провайдеров.
  Рекомендую **OpenAI** (то же, что ChatGPT). Альтернатива бесплатно —
  **Gemini**. См. `SETUP.md` шаг 5. Без ключа AI-OCR-переключатель
  заблокирован, Tesseract остаётся как fallback.
- **«О разработке» — финальный контент:** доделываем онлайн в чате.
- **LLM-чат «обсудить рецепт»** — отложено на отдельную итерацию.

## Настройка инфраструктуры

Все шаги по Drive / Sheets / Vision вынесены в [`SETUP.md`](SETUP.md):
- Шаг 1 · Сервисный аккаунт Google.
- Шаг 2 · Папка Drive для рецептов (рецепт = Google Doc + JSON).
- Шаг 3 · Мастер-таблица Sheets (рецепты ровной таблицей).
- Шаг 4 · Secrets в Streamlit Cloud.
