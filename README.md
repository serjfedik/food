...
- **Anthropic API key** — добавил поддержку Claude Vision для вкладки
  «фото холодильника (ai)» в разделе Холодильник. Активируется, как только
  вы добавите ключ в Streamlit Cloud → Settings → Secrets:
  `ANTHROPIC_API_KEY = "sk-ant-..."`. До этого вкладка покажет инструкцию.
- **«О разработке» — финальный контент:** жду ваш MD с бизнес-целями,
  конкурентами и именем автора. Структура уже под скриншот: блок
  статистики переделан на 3 карточки (ВЕРСИЯ / ОБНОВЛЕНО / ДАТА ОТСЧЁТА)
  с датами в формате DD.MM.YYYY.
- **Внешние рецепты на русском без LLM** — реалистичных бесплатных API
  с русскими рецептами на рынке нет (см. ответ ниже). Сейчас оптимальный
  путь — расширять `data/starter_recipes.json` (20 → 50–100 проверенных).
  Скажите, делаем — за один заход добавлю.
- **LLM-чат «обсудить рецепт»** — отложено по вашей просьбе на отдельную
  итерацию.

## Подключить Claude Vision (для фото холодильника)

1. Зарегистрироваться на https://console.anthropic.com.
2. Раздел **API Keys** → **Create Key** → скопировать `sk-ant-...`.
3. Положить на счёт $5-10 (одно распознавание ≈ $0.005 на Haiku).
4. В Streamlit Cloud открыть приложение → **Settings → Secrets**, добавить:
   ```
   ANTHROPIC_API_KEY = "sk-ant-..."
   ```
5. Сохранить — приложение перезагрузится, вкладка «фото холодильника (ai)»
   станет рабочей.

Используется модель **Claude Haiku 4.5** по умолчанию (быстрая и дешёвая).
В UI можно переключить на **Sonnet 4.6** (баланс) или **Opus 4.8** (максимум).
